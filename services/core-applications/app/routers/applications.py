...
async def get_applications(
    user_id: UUID, type: str | None = Query(None), id: UUID | None = Query(None), db: AsyncSession = Depends(get_db)
):
    if id:
        result = await db.execute(
            select(UserApplication).where(UserApplication.id == id, UserApplication.user_id == user_id)
        )
        ...
        return (
            [
                ApplicationResponse(
                    ...
                    notifications_count=application.notifications_count,
                )
            ]
            if application
            else []
    ...
    return [
        ApplicationResponse(
            ...
            notifications_count=a.notifications_count,
        )
...
async def get_notifications(
    ...
    query = (
        ...
        .join(UserApplication, ApplicationNotification.application_id == UserApplication.id)
