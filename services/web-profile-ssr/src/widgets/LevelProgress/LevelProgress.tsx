...
export const LevelProgress = ({ levelInfo }: Props) => {
  ...
  return (
    <Paper className={styles.container} elevation={3} sx={{background: `linear-gradient(135deg, ${levelInfo.color}22 0%, ${levelInfo.color}44 100%)`}}>
      ...
      <Box className={styles.progressSection}>
        ...
        {!isMaxLevel ? (
          <LinearProgress variant="determinate" value={progress} sx={{height: 8, borderRadius: 4, backgroundColor: `${levelInfo.color}33`, '& .MuiLinearProgress-bar': {backgroundColor: levelInfo.color}}} />
        ) : (
          <Typography variant="caption" sx={{color: levelInfo.color, fontWeight: 600, textAlign: 'center', marginTop: 1}}>
