...
interface Route {
  ...
  /** Открыт ли маршрут снаружи через ingress. Вручную поддерживается. По умолчанию: true */
  public?: boolean;
}
...
/** Read existing router.yaml and return a map of "path:method" → public */
function loadExistingPublicValues(outputPath: string): Map<string, boolean> {
  ...
  try {
    ...
    for (const route of parsed?.routes ?? []) {
      if (route.public !== undefined) {
        map.set(`${route.path}:${route.method ?? ''}`, route.public);
      }
    }
  } catch {
    // ignore parse errors — start fresh
...
export async function generateRouterConfigs(): Promise<void> {
  ...
  for (const service of nestServices) {
    try {
      ...
      // Preserve manually-set auth_level and public values from existing router.yaml
      const existingAuthLevels = loadExistingAuthLevels(outputPath);
      const existingPublicValues = loadExistingPublicValues(outputPath);
      ...
      // Apply preserved auth_levels and public (or defaults)
      routes = routes.map((r) => ({
        ...r,
        auth_level: existingAuthLevels.get(`${r.path}:${r.method ?? ''}`) ?? r.auth_level ?? 'none',
        public: existingPublicValues.get(`${r.path}:${r.method ?? ''}`) ?? true,
      }));
      ...
      const fileHeader = `# Generated router configuration for ${service.name} service\n# auth_level and public are manually maintained — use pnpm run generate:router to update routes.\n\n`;
