# Генератор инфраструктуры

Supreme Infrastructure Generator - это всеобъемлющий инструмент, который автоматизирует создание и поддержку всех файлов конфигурации инфраструктуры для монопрепозитория supreme-infra.

## Обзор

Генератор расположен в директории `infra/generate/` и состоит из нескольких специализированных генераторов, которые работают вместе для поддержания согласованности во всех компонентах инфраструктуры:

1. **Генератор конфигурации роутера** - Генерирует конфигурации роутера сервисов
2. **Обновлятор проверок безопасности** - Обновляет GitHub Actions workflows для сканирования безопасности
3. **Обновлятор CD workflow** - Обновляет workflows непрерывного развертывания
4. **Генератор файлов values** - Генерирует values файлы Helm чартов для всех окружений

## Как это работает

### Главная точка входа

Главный генератор (`infra/generate/index.ts`) оркестрирует все суб-генераторы в правильном порядке:

```typescript
console.log('📋 Step 1/4: Generating router configurations...');
generateRouterConfigs();

console.log('📋 Step 2/4: Updating security checks...');
updateSecurityChecks();

console.log('📋 Step 3/4: Updating CD workflow...');
updateCdWorkflow();

console.log('📋 Step 4/4: Generating values files...');
generateValuesForAllServices();
```

### Источник конфигурации

Все генераторы используют единый источник правды: `services.yaml` в корне проекта:

```yaml
services:
  nest:
    - name: backend
      description: Backend service (NestJS)

  next:
    - name: frontend
      description: Frontend service (Next.js)
```

## Суб-генераторы

### 1. Генератор конфигурации роутера

**Расположение**: `infra/generate/generate-router/`

**Назначение**: Генерирует конфигурации роутера для сервисов на основе их NestJS контроллеров.

**Что делает**:

- Сканирует NestJS сервисы на файлы контроллеров
- Извлекает информацию о роутах из декораторов
- Генерирует файлы конфигурации роутера
- Обновляет определения роутера сервиса

**Генерируемые файлы**:

- `services/{service}/router.yaml`

### 2. Обновлятор проверок безопасности

**Расположение**: `infra/generate/update-security-checks/`

**Назначение**: Обновляет GitHub Actions workflows проверок безопасности для включения всех сервисов.

**Что делает**:

- Читает список сервисов из `services.yaml`
- Обновляет `.github/workflows/security-checks.yml`
- Добавляет NestJS сервисы в матрицу `security-scan-nest`
- Добавляет Next.js сервисы в матрицу `security-scan-next`

### 3. Обновлятор CD workflow

**Расположение**: `infra/generate/update-cd-workflow/`

**Назначение**: Обновляет workflow непрерывного развертывания текущим списком сервисов.

**Что делает**:

- Читает список сервисов из `services.yaml`
- Обновляет `.github/workflows/cd.yml`
- Добавляет все сервисы в селектор ручного развертывания
- Устанавливает первый сервис по умолчанию

### 4. Генератор файлов values

**Расположение**: `infra/generate/generate-overrides/`

**Назначение**: Генерирует полные файлы values Helm чартов для всех сервисов и окружений.

**Что делает**:

- Читает конфигурации сервисов из `services/{service}/service.yaml`
- Применяет значения по умолчанию из `infra/helmcharts/{service-type}/values.yaml`
- Применяет специфические для окружения overrides из `infra/helmcharts/{service-type}/environment-overrides.yaml`
- Применяет специфические для сервиса overrides из `services/{service}/service.yaml`
- Генерирует финальные файлы values для каждого окружения

**Генерируемые файлы**:

- `infra/overrides/development/{service}.yaml`
- `infra/overrides/production/{service}.yaml`

## Использование

### Запуск всех генераторов

```bash
# Из корня проекта
pnpm run generate
```

Это запускает все генераторы последовательно и предоставляет подробный вывод:

```
═══════════════════════════════════════════════════════════
🚀 Supreme Infrastructure Generator
═══════════════════════════════════════════════════════════

📋 Step 1/4: Generating router configurations...
───────────────────────────────────────────────────────────
✓ Router configurations updated successfully!

📋 Step 2/4: Updating security checks...
───────────────────────────────────────────────────────────
✓ Security checks updated successfully!
  Nest services: backend
  Next services: frontend

📋 Step 3/4: Updating CD workflow...
───────────────────────────────────────────────────────────
✓ CD workflow updated successfully!
  Services: backend, frontend
  Default service: backend

📋 Step 4/4: Generating values files...
───────────────────────────────────────────────────────────
→ Starting values generation process
→ Found 2 service(s) to process:
→   • backend (nest) - Backend service (NestJS)
→   • frontend (next) - Frontend service (Next.js)

[... подробные логи генерации ...]

→ Generation summary:
✓   Total files generated: 6

═══════════════════════════════════════════════════════════
✅ All generation tasks completed successfully!
═══════════════════════════════════════════════════════════
```

### Запуск отдельных генераторов

Вы также можете запускать генераторы по отдельности:

```bash
# Только конфигурации роутера
pnpm run generate:router

# Только проверки безопасности и CD workflow
pnpm run infra:generate

# Только файлы values
pnpm run generate:overrides
```

## Добавление нового сервиса

Чтобы добавить новый сервис в инфраструктуру:

1. **Добавить в services.yaml**:

```yaml
services:
  nest:
    - name: backend
      description: Backend service (NestJS)
    - name: my-new-service
      description: My new NestJS service

  next:
    - name: frontend
      description: Frontend service (Next.js)
```

2. **Создать конфигурацию сервиса**:
   Создать `services/my-new-service/service.yaml` с настройками конкретного сервиса:

```yaml
# Service-specific Helm overrides
image:
  tag: 'latest'

resources:
  requests:
    cpu: '100m'
    memory: '128Mi'
  limits:
    cpu: '500m'
    memory: '512Mi'

autoscaling:
  enabled: true
  minReplicas: 1
  maxReplicas: 5
```

3. **Создать структуру директорий сервиса**:

```
services/my-new-service/
├── src/
├── package.json
├── Dockerfile
├── service.yaml    # Конфигурация сервиса
└── router.yaml     # Генерируется генератором роутера
```

4. **Запустить генератор**:

```bash
pnpm run generate
```

Генератор автоматически:

- Добавит сервис в workflows проверок безопасности
- Добавит сервис в опции развертывания CD
- Сгенерирует файлы values Helm для всех окружений
- Создаст конфигурацию роутера

## Файлы конфигурации

### services.yaml (Единый источник правды)

```yaml
services:
  nest: # NestJS сервисы
    - name: backend
      description: Backend service (NestJS)

  next: # Next.js сервисы
    - name: frontend
      description: Frontend service (Next.js)
```

### Конфигурация сервиса (service.yaml)

Каждый сервис имеет свой собственный файл конфигурации, который определяет специфические для сервиса overrides:

```yaml
# services/backend/service.yaml
image:
  tag: 'latest'

service:
  port: 4000
  targetPort: 4000

env:
  NODE_ENV: 'production'
  PORT: '4000'

resources:
  requests:
    cpu: '250m'
    memory: '256Mi'
  limits:
    cpu: '1000m'
    memory: '1Gi'

autoscaling:
  enabled: true
  minReplicas: 2
  maxReplicas: 10
```

## Directory Structure

```
infra/
├── generate/                    # Main generator directory
│   ├── index.ts                # Main entry point (super-generator)
│   ├── README.md               # This documentation
│   ├── generate-router/        # Router configuration generator
│   │   ├── index.ts
│   │   ├── extract-nest-routes.ts
│   │   └── extract-next-routes.ts
│   ├── update-security-checks/ # Security checks workflow updater
│   │   └── index.ts
│   ├── update-cd-workflow/     # CD workflow updater
│   │   └── index.ts
│   └── generate-overrides/     # Values files generator
│       ├── generate-overrides.ts
│       ├── index.ts
│       ├── README.md
│       └── types.ts
├── helmcharts/                 # Helm chart templates
│   ├── backend-service/
│   └── frontend-service/
├── overrides/                  # Generated values files
│   ├── development/
│   │   ├── backend.yaml
│   │   └── frontend.yaml
│   └── production/
│       ├── backend.yaml
│       └── frontend.yaml
└── shared/                     # Shared utilities
    └── load-services.ts        # Service loading utility
```

## Преимущества

- **Единый источник правды**: Вся конфигурация приходит из `services.yaml`
- **Автоматизированная поддержка**: Нет ручных обновлений workflows или конфигов
- **Согласованность**: Гарантирует синхронизацию всех компонентов инфраструктуры
- **Предотвращение ошибок**: Снижает ошибки ручной конфигурации
- **Разработчик опыт**: Одна команда для обновления всего

## Когда запускать генератор

Запускайте генератор после:

- Добавления нового сервиса
- Изменения конфигурации сервиса в `service.yaml`
- Модификации значений по умолчанию в Helm чартах
- Обновления environment overrides
- Добавления новых окружений

## Устранение неисправностей

### Ошибка "No services found"

Убедитесь, что `services.yaml` существует и содержит хотя бы один сервис:

```yaml
services:
  nest:
    - name: backend
      description: Backend service (NestJS)
```

### Ошибки обновления workflows

Проверьте, что файлы workflows существуют и имеют правильную структуру:

- `.github/workflows/security-checks.yml`
- `.github/workflows/cd.yml`

### Ошибки генерации values

Убедитесь, что каждый сервис имеет файл `service.yaml`:

- `services/{service_name}/service.yaml`

## Интеграция с CI/CD

Генератор разработан для бесшовной работы с CI/CD pipeline:

1. **Pre-deployment**: Генератор запускается для обеспечения актуальности всех конфигов
2. **Security Checks**: Обновленные workflows включают все сервисы
3. **Deployment**: CD workflow имеет текущий список сервисов
4. **Helm Deployment**: Сгенерированные файлы values используются для развертывания

Это гарантирует, что любые изменения инфраструктуры автоматически отражаются в pipeline развертывания.
